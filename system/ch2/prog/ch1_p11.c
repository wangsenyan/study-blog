...
char **lsh_split_line(char *line)
{
  ...
  //C 库函数 char *strtok(char *str, const char *delim) 分解字符串 str 为一组字符串，delim 为分隔符。返回分解的第一个字符串
  while (token != NULL)
  {
    ...
    token = strtok(NULL, LSH_TOK_DELIM); //得到其他字符串
  }
  if (position > 0 && strcmp(tokens[position - 1], "&") == 0) //是否后台运行
  {
    conc = true;
