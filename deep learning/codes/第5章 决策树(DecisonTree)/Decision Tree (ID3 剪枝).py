# numpy N为数组对象,精密广播功能函数,集成C/C++和Fortran代码的工具,强大的线性代数,傅里叶变换和随机数功能
# pandas 分析结构化数据的工具集,基础是Numpy,用于数据挖掘和数据分析,同时也提供数据清洗功能
# pandas DataFrame 表格型数据结构
# pandas Series 类似于一维数组的对象
import numpy as np
import pandas as pd
    ...
    def train(self, datasets, node):
        ...
        for Di in vc.index:
            ...
            new_datasets = pd.DataFrame(
                [list(i) for i in datasets.values if i[gainmaxi] == Di], columns=datasets.columns)
        ...
        if node.label is not None:
            ...
            if None not in cy:  # 全是叶节点
                ...
                while i < len(node.child):
                    if_re = self.findfather(
                        node.child[i], errormin)  # 若剪过枝，则其父节点要重新检测
                    if if_re == 1:
                        re = 1
                    elif if_re == 2:
                        i -= 1
